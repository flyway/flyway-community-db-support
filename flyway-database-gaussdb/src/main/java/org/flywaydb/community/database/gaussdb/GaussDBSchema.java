...
/**
 * GaussDB implementation of Schema.
 * @author chen zhida
 * Notes: Original code of this class is based on PostgreSQLSchema.
 */
public class GaussDBSchema extends Schema<GaussDBDatabase, GaussDBTable> {
    /**
     * Creates a new GaussDB schema.
     *
     * @param jdbcTemplate The Jdbc Template for communicating with the DB.
     * @param database The database-specific support.
     * @param name The name of the schema.
     */
    protected GaussDBSchema(JdbcTemplate jdbcTemplate, GaussDBDatabase database, String name) {
    ...
    private List<String> generateDropStatementsForRoutines() throws SQLException {
        // #2193: GaussDB 11 removed the 'proisagg' column and replaced it with 'prokind'.
        String isAggregate = database.getVersion().isAtLeast("11") ? "pg_proc.prokind = 'a'" : "pg_proc.proisagg";
        // PROCEDURE is only available from GaussDB 11
    ...
    @Override
    protected GaussDBTable[] doAllTables() throws SQLException {
        ...
        GaussDBTable[] tables = new GaussDBTable[tableNames.size()];
        for (int i = 0; i < tableNames.size(); i++) {
            tables[i] = new GaussDBTable(jdbcTemplate, database, this, tableNames.get(i));
        }
        return tables;
    }
    ...
    public Table getTable(String tableName) {
        return new GaussDBTable(jdbcTemplate, database, this, tableName);
    }

    @Override
    protected Type getType(String typeName) {
        return new GaussDBType(jdbcTemplate, database, this, typeName);
    }
}
