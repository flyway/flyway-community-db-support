...
public class DorisSchema extends Schema<DorisDatabase, DorisTable> {

    DorisSchema(JdbcTemplate jdbcTemplate, DorisDatabase database, String name) {
    ...
    private List<String> cleanRoutines() throws SQLException {
        List<Map<String, String>> routineNames = jdbcTemplate.queryForList(
    ...
    private List<String> cleanSequences() throws SQLException {
        ...
        for (String sequenceName : names) {
            statements.add("DROP SEQUENCE " + database.quote(this.name, sequenceName));
        }
        return statements;
    }

    @Override
    protected DorisTable[] doAllTables() throws SQLException {
        List<String> tableNames = jdbcTemplate.queryForStringList(
                "SELECT table_name FROM information_schema.tables WHERE table_schema=?" +
                        " AND table_type IN ('BASE TABLE', 'SYSTEM VERSIONED')", name);

        DorisTable[] tables = new DorisTable[tableNames.size()];
        for (int i = 0; i < tableNames.size(); i++) {
            tables[i] = new DorisTable(jdbcTemplate, database, this, tableNames.get(i));
        }
        return tables;
    }
    ...
    public Table getTable(String tableName) {
        return new DorisTable(jdbcTemplate, database, this, tableName);
    }
}
